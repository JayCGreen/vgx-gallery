...
export async function getCollections(){
    var res = await getCloudflareContext(true).env.vgx_feed.prepare("SELECT * FROM Collections").run()
    return res.results;
}

export async function getTags(){
    var res = await getCloudflareContext(true).env.vgx_feed.prepare("SELECT * FROM Tags").run();
    return res.results;
}
